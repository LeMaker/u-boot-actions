...
int check_key(void)
{
    ...
    /*Need to use the v1.3 version of the Guitar Base Board*/
	writel(readl(GPIO_BOUTEN) & ~(1 << 9), GPIO_BOUTEN);
	writel(readl(GPIO_BINEN) | (1 << 9), GPIO_BINEN);
	if(!(readl(GPIO_BDAT) & 0x200))
	{
		reset_to_adfu();
	}
	key = adckey_scan(); 
